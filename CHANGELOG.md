...
- **Опциональный выбор LLM-провайдера в онбординге.** Раньше Convoic принудительно скачивал локальную LLM (1.2–2.6 GiB) независимо от того, планирует ли пользователь использовать локальную модель или облачного провайдера. Теперь на отдельном шаге онбординга пользователь выбирает стратегию генерации саммари:
  - **Локальная модель** — скачивается рекомендованная по RAM Qwen-модель, всё работает офлайн. Поведение для пользователей, выбирающих local, идентично прежнему.
  - **Облачный провайдер** — Caila (рекомендуется для российских пользователей, предустановлена по умолчанию), OpenAI, Claude, OpenRouter или Custom OpenAI. Ничего не скачивается — достаточно ввести API-ключ и выбрать модель. Доступна кнопка «Проверить подключение» (для Caila) для валидации ключа перед сохранением.
  - **Пропустить** — отложить выбор. Модель распознавания речи (GigaAM, ~227 MB) скачивается в любом случае. При первой попытке сгенерировать резюме приложение открывает диалог настройки (lazy-gate) и просит выбрать провайдера.
  - **Архитектура:** discriminated-union `OnboardingProviderChoice { Local | Cloud | Skip }` на Rust с `#[serde(tag = "kind")]` (компилятор ловит пропущенные ветви на обеих сторонах), зеркальный `SummaryProviderDecision` на TypeScript, новый шаг `ProviderChoiceStep` в flow (теперь 5 шагов вместо 4), условный рендеринг в `DownloadProgressStep`, единый `ProviderSetupGateContext` для глобального открытия диалога настроек (заменил 3 разрозненных механизма).
  - **DB-миграция:** колонки `onboarding_provider_choice` и `provider_chosen_at` (additive, `provider TEXT NOT NULL` не тронут). Маркер `"deferred"` триггерит lazy-gate.
  - **Lazy-gate** при первом саммари работает проактивно (проверка до invoke) и реактивно (fallback на backend-ошибку «model is required») как defense-in-depth.
...
- **Новая LLM-модель в каталоге — RuadaptQwen3-4B (Russian-optimized).** Russian-adapted Qwen3-4B-Instruct-2507 от [RefalMachine](https://huggingface.co/RefalMachine/RuadaptQwen3-4B-Instruct) с continued pre-training на русском корпусе и расширенным токенайзером (+48K русских токенов). Заменяет legacy-вариант Gemma 3 4B в каталоге моделей саммаризации.
  ...
  - **Размер:** 2.49 GB (Q4_K_M GGUF), почти равен Qwen 3.5 4B (2.55 GB)
  - **Дефолт:** Qwen 3.5 4B остаётся дефолтом. Ruadapt доступен как опция для тех, кто предпочитает более структурированный вывод (с пониманием риска галлюцинаций в Owner-колонке таблиц Action Items на зашумлённых транскриптах).

### 🐛 Исправлено

- **Критический баг первого запуска: белый экран, кнопки не работают.** На первом запуске Convoic `initialize_database_on_startup` не создавал БД синхронно, а только эмитил событие `first-launch-detected`, которое никто на фронтенде не слушал. В результате `AppState` никогда не регистрировался через `app.manage()`, и команды `api_get_transcript_config` / `api_get_model_config` / `api_get_meetings` падали с `state not managed for field 'state'`. Симптомы: сайдбар рисуется, основная область белая, кнопки не реагируют. Теперь БД и `AppState` инициализируются синхронно всегда (и на first launch, и на повторных запусках).

### 🔒 Анти-галлюцинационный guard в системном промпте

- **Правило №10 в `build_final_report_system_prompt`** (применяется ко всем моделям и шаблонам): таблицы Action Items и подобные запрещено заполнять выдуманными данными. Каждая ячейка (Owner, Due, timestamp) должна дословно из транскрипта; если данных нет — модель обязана писать «Не указан». Запрещены выдуманные роли вроде «Архитектор платформы» или «Финансовый менеджер». Выявлено в ходе A/B теста Ruadapt vs Qwen 3.5 4B на реальной записи: обе модели на зашумлённом STT-входе добросовестно заполняли Owner/Due. После фикса Qwen 3.5 4B честно пишет «Не указан» во всех пустых полях.

### 🛠️ Технически

- **Документация dev-workflow** (`CLAUDE.md`): добавлено обязательное правило очистки **двух** кэшей перед `pnpm tauri:dev` — Next.js `.next/` и WebView2 `%LOCALAPPDATA%\com.convoic.app\EBWebView\Default\`. Второй критичен: WebView2 агрессивно кэширует JS-бандлы по URL и игнорирует обновлённый файл на диске, что проявляется как `layout.js:NNN SyntaxError: Invalid or unexpected token` при синтаксически валидном файле.
- Обновлён приоритет моделей в `summary_model_priority`: `qwen3.5:4b`=5 → `ruadapt-qwen3:4b`=4 → `qwen3.5:2b`=3 → `gemma3:1b`=1. Убранная из каталога `gemma3:4b` корректно деградирует до приоритета 0 (graceful fallback для существующих БД).
