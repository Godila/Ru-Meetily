...
export function useRecordingStart(
  ...
  // Check if the selected transcription model is ready. Provider-aware so that
  // a GigaAM-only (or Parakeet-only) user is not gated by the other engine.
  const checkModelReady = useCallback(async (): Promise<boolean> => {
    ...
    try {
      if (provider === 'gigaam') {
        ...
        return await invoke<boolean>('gigaam_has_available_models');
      }
      if (provider === 'localWhisper') {
        await invoke('whisper_init');
        return await invoke<boolean>('whisper_has_available_models');
      }
      // parakeet (and anything else as a safe default)
  ...
  // Check if the selected provider's model is currently downloading.
  const checkIfModelDownloading = useCallback(async (): Promise<boolean> => {
    const provider = transcriptModelConfig?.provider || 'parakeet';
    const cmd = provider === 'gigaam'
      ? 'gigaam_get_available_models'
