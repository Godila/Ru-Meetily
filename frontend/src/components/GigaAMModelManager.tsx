...
export function GigaAMModelManager({
  ...
  useEffect(() => {
    ...
    const initializeModels = async () => {
      try {
        ...
        toast.error('Failed to load GigaAM models', {
  ...
  // Download progress / complete / error listeners.
  useEffect(() => {
    ...
    const setupListeners = async () => {
      ...
      unlistenComplete = await listen<{ modelName: string }>(
        ...
        (event) => {
          ...
          toast.success(`${displayInfo?.icon || '✓'} ${displayName} готов!`, {
            description: 'Модель загружена и готова к использованию',
            duration: 4000
          });
  ...
  const cancelDownload = async (modelName: string) => {
    ...
    try {
      ...
      setModels(prevModels =>
        prevModels.map(model =>
          model.name === modelName ? { ...model, status: 'Missing' as ModelStatus } : model
        )
      );
      progressThrottleRef.current.delete(modelName);
  ...
  const selectModel = async (modelName: string) => {
    ...
    const displayInfo = getModelDisplayInfo(modelName);
    const displayName = displayInfo?.friendlyName || modelName;
    toast.success(`Переключено на ${displayName}`, { duration: 3000 });
  };
  ...
  if (error) {
    return (
      <div className={`bg-red-50 border border-red-200 rounded-lg p-4 ${className}`}>
        <p className="text-sm text-red-800">Не удалось загрузить модели</p>
        <p className="text-xs text-red-600 mt-1">{error}</p>
      </div>
    );
  }

  const recommendedModel = models.find(m => m.name === 'gigaam-v3-rnnt-int8');
  const otherModels = models.filter(m => m.name !== 'gigaam-v3-rnnt-int8');
