  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">Аудиоустройства</h4>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">Микрофоны не найдены</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">Уровни микрофона:</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">Системные аудиоустройства не найдены</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>Микрофон:</strong> Записывает ваш голос и окружающий звук</p>
        <p>• <strong>Системный звук:</strong> Записывает звук компьютера (музыку, звонки и т.д.)</p>
        {isMonitoring && (
          <p>• <strong>Уровни микрофона:</strong> Зелёный = хорошо, жёлтый = громко, красный = слишком громко</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>Совет:</strong> Нажмите «Тест микрофона», чтобы проверить работу микрофона</p>
        )}
      </div>
    </div>
  );
