...
export const AISummary = ({ summary, status, error, onSummaryChange, onRegenerateSummary, meeting }: Props) => {
  ...
  const currentSummary = useMemo(() => {
    if (!summary) {
      return {
        Agenda: { title: "Повестка", blocks: [] },
        Decisions: { title: "Решения", blocks: [] },
        ActionItems: { title: "Задачи", blocks: [] },
        ClosingRemarks: { title: "Заключительные замечания", blocks: [] }
      };
    }
  ...
  const renderErrorState = () => (
    <div className="w-full p-4 bg-red-50 border border-red-200 rounded-lg">
      <div className="flex items-center mb-2">
        ...
        <h3 className="text-red-700 font-medium">Ошибка генерации резюме</h3>
      </div>
      <p className="text-red-600 text-sm">{error}</p>
      <p className="text-red-500 text-xs mt-2">Проверьте конфигурацию модели и API-ключи или попробуйте снова.</p>
    </div>
  );
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h3 className="text-blue-700 font-medium">
            {status === 'processing' ? 'Обработка транскрипции' : 'Генерация резюме'}
          </h3>
          <p className="text-blue-600 text-sm">
            {status === 'processing'
              ? 'Анализ транскрипции...'
              : 'Создание подробного резюме встречи...'}
          </p>
        </div>
      </div>
    </div>
  ...
  if (!hasContent && status === 'completed') {
    return (
      <div className="w-full p-4 bg-gray-50 border border-gray-200 rounded-lg text-center">
        <p className="text-gray-600">Содержимое резюме недоступно.</p>
        <p className="text-gray-500 text-sm mt-1">Попробуйте сгенерировать новое резюме.</p>
      </div>
    );
  }
      ...
      {contextMenu.visible && selectedBlocks.length > 0 && (
        ...
        >
          ...
          >
            ...
            <span>Копировать {selectedBlocks.length > 1 ? `${selectedBlocks.length} блоков` : 'блок'}</span>
          </button>
          <button
          ...
          >
            <span>🗑️</span>
            <span>Удалить {selectedBlocks.length > 1 ? `${selectedBlocks.length} блоков` : 'блок'}</span>
          </button>
        </div>
      )}
