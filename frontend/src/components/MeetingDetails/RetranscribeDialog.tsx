...
export function RetranscribeDialog({
  ...
  const isLanguageFixedModel = selectedModelDetails?.provider === 'parakeet' || selectedModelDetails?.provider === 'gigaam';
  ...
  const handleStartRetranscription = async () => {
    ...
    try {
      const languageToSend = isLanguageFixedModel ? null : selectedLang === 'auto' ? null : selectedLang;
      await Analytics.track('enhance_transcript_started', {
        language: isLanguageFixedModel ? 'auto' : (selectedLang === 'auto' ? 'auto' : selectedLang),
          ...
          <DialogDescription>
            {isProcessing
              ...
              : error
                ...
                : 'Повторная обработка аудио с другими настройками языка'}
          </DialogDescription>
        </DialogHeader>
