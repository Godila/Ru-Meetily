...
fn main() {
    ...
    let skip_ffmpeg = std::env::var("CONVOIC_SKIP_SIDECAR_VERIFY").map_or(false, |v| v == "1");

    if !skip_ffmpeg {
        ffmpeg::ensure_ffmpeg_binary();
    } else {
        println!("cargo:warning=⏭️  Skipping FFmpeg download (CONVOIC_SKIP_SIDECAR_VERIFY=1).");
    }
    tauri_build::build()
}
...
fn detect_and_report_gpu_capabilities() {
    ...
    println!("cargo:warning=🚀 Building Convoic for: {}", target_os);
