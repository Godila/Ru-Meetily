...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\convoic-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("convoic-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("convoic-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/convoic-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("convoic-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/convoic-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("convoic-recordings")
    }
