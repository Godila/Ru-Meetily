...
const RUSSIAN_BASE_SUMMARY_INSTRUCTION: &str =
    "**Пиши саммари/отчёт на русском языке; текст на других языках недопустим.**";
...
1. {RUSSIAN_BASE_SUMMARY_INSTRUCTION}
...
5. If a section has no relevant info, write "В этом разделе ничего не отмечено."
...
/// * `force_cpu` - Force CPU inference (BuiltInAI provider, GPU toggle off)
///
/// # Returns
/// Tuple of (final_summary_markdown, number_of_chunks_processed).
/// Output language is Russian for all providers (see RUSSIAN_BASE_SUMMARY_INSTRUCTION).
...
mod tests {
    ...
    #[test]
    fn chunk_summary_prompt_forces_russian_base_output() {
        let prompt = build_chunk_summary_user_prompt("Содержание встречи");

        assert!(prompt.contains(RUSSIAN_BASE_SUMMARY_INSTRUCTION));
        assert!(prompt.contains("<transcript_chunk>"));
    }

    #[test]
    fn combine_summary_prompt_forces_russian_base_output() {
    ...
    #[test]
    fn final_report_prompt_forces_russian_base_output_and_translates_headings() {
    ...
    #[test]
    fn russian_base_instruction_marks_non_russian_prose_invalid_without_bloat() {
        assert!(RUSSIAN_BASE_SUMMARY_INSTRUCTION.contains("на русском языке"));
        assert!(RUSSIAN_BASE_SUMMARY_INSTRUCTION.contains("недопустим"));
        // Keep prompt-size guard to catch accidental expansion that bloats every call site
        assert!(RUSSIAN_BASE_SUMMARY_INSTRUCTION.len() <= 160);
    }
}
